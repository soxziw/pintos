...
pintos_init (void)
{
  ...
  if (*argv != NULL) {
    ...
    static const uint8_t INPUT_EXIT[64]={'e','x','i','t'};
    static const uint8_t INPUT_WHOAMI[64]={'w','h','o','a','m','i'};
    static uint8_t input_tmp[64]={};
    static const char OUTPUT_STUDENTID[64]="2000012915";
    static const char OUTPUT_INVALID[64]="invalid command";
    int in_cnt=0;
    while(1){
      in_cnt=0;
      printf("PKUOS>");
      input_init();
      uint8_t in_char =input_getc();
      while(in_char!='\r'){
        input_tmp[in_cnt++]=in_char;
        putchar(in_char);
        input_init();
        in_char=input_getc();
      }
      putchar('\n');
      if(!memcmp(INPUT_EXIT,input_tmp,sizeof(INPUT_EXIT))){
        break;
      }
      else if(!memcmp(INPUT_WHOAMI,input_tmp,sizeof(INPUT_WHOAMI))){
        puts(OUTPUT_STUDENTID);
      }
      else{
        puts(OUTPUT_INVALID);
      }
